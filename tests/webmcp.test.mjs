...
describe('webmcp.ts: current API contract', () => {
  ...
  it('switches every monitor key and rejects unknown or malformed destinations', async () => {
    ...
    assert.deepEqual(tool.inputSchema.properties.monitor.enum, [
      'full', 'tech', 'finance', 'happy', 'commodity', 'energy', 'chile',
    ]);
    assert.equal(tool.annotations.readOnlyHint, false);

    const signal = new AbortController().signal;
    for (const monitor of ['full', 'tech', 'finance', 'commodity', 'energy', 'happy', 'chile']) {
    ...
    assert.deepEqual(switches, ['full', 'tech', 'finance', 'commodity', 'energy', 'happy', 'chile']);
