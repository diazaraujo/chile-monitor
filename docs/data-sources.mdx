...
| `chile` | `politics` | Google News Chile (es); BioBio (es); Emol (es); La Tercera (es); Cooperativa (es); El Mostrador (es); T13 (es); CNN Chile (es); 24 Horas (es); ADN Radio (es); Diario Financiero (es); La Segunda (es); The Clinic (es); El Desconcierto (es); CIPER (es) |
| `chile` | `us` | Prensa Chile (es); La Tercera Política (es); Ex-Ante (es); El Líbero (es); Interferencia (es); Radio U. de Chile (es); Senado (es); Cámara de Diputados (es); La Moneda (es) |
| `chile` | `gov` | SEA / SEIA (es); SMA (es); Tribunales Ambientales (es); DGA / agua (es); Diario Oficial (es); Contraloría (es); Corte Suprema (es); CMF (es); SERNAGEOMIN / minería (es); Municipalidades (es) |
| `chile` | `climate` | Clima Chile (es); Incendios CONAF (es); SENAPRED (es); Meteorología (es); Ladera Sur (es); País Circular (es); MMA (es); Pueblos originarios (es) |
| `chile` | `latam` | Cono Sur (es); Regiones (SoyChile) (es); El Día (Coquimbo) (es); Mercurio de Valparaíso (es); El Sur (Concepción) (es); Austral (Temuco/Valdivia) (es); El Pingüino (Magallanes) (es) |
